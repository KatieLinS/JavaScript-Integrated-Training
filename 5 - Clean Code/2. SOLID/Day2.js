function IntcodeComputer(intString) {
  const intArray = intString.split(",").map((str) => parseInt(str, 10));
  for (let i = 0; i < intArray.length; i += 4) {
    if (intArray[i] === 99){
      return intArray;
    }
    inputPos1 = intArray[i + 1];
    inputPos2 = intArray[i + 2];
    outputPos = intArray[i + 3];

    switch (intArray[i]) {
      case 1:
        intArray[outputPos] = intArray[inputPos1] + intArray[inputPos2]; 
        break;
      case 2:
        intArray[outputPos] = intArray[inputPos1] * intArray[inputPos2]; 
        break;
...
const input =
  ...
  console.dir(IntcodeComputer(input)[0]);
